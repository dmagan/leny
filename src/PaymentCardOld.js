...
<div className="mt-1">
  ...
  <p className="text-[#f7d55d] font-bold mt-1 text-3xl">
    {price} $
            ...
            {/* انتخابگر روش پرداخت */}
<div className="mb-4">
  <p className={`text-2xl mb-3 text-center ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}dir='rtl'>
  ...
  <div className="flex gap-2 mb-4">
    ...
    >
      <div className="text-center">
        <div className="font-bold">TRON (TRC20)</div>
        <div className="text-xs opacity-80">USDT</div>
      </div>
    </button>
    ...
    >
      <div className="text-center">
        ...
        <div className="text-xs opacity-80">USDT / SOL</div>
      </div>
    </button>
  </div>
</div>
...
<div className="mt-2 mb-4 p-4 rounded-xl border border-yellow-500/90">
  <p className="text-gray-100 text-sm text-right mb-4 font-bold" dir="rtl">
    {paymentMethod === 'tron' ? (
      ...
      <>برای خرید اشتراک می‌بایست به آدرس ولت زیر <span className="text-blue-400">تتر یا سولانا</span> بر روی <span className="text-blue-400">شبکه سولانا</span> ارسال کنید:</>
    )}
  </p>
  ...
  <div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg mb-2">
    ...
    <div className="font-mono text-[12px] text-yellow-500 select-all font-bold break-all" onClick={copyToClipboard}>
  ...
  <div className="text-xs text-right text-gray-100 font-bold" dir="rtl">
    {paymentMethod === 'tron' ? (
      <>
        <p className="mb-1 text-red-500 font-bold">دقت کنید درصورتی که هر ارز دیگری غیر از USDT یا بر روی شبکه اشتباه ارسال کنید مورد قبول نیست.</p>
        <p className="mb-1 font-bold">ترجیحا از صرافی‌های ایرانی اقدام به پرداخت نکنید.</p>
      </>
    ) : (
      <>
        <p className="mb-1 text-blue-400 font-bold">می‌توانید USDT یا SOL بر روی شبکه سولانا ارسال کنید.</p>
        <p className="mb-1 font-bold">مطمئن شوید که شبکه سولانا را انتخاب کرده‌اید.</p>
      </>
    )}
...
placeholder="هش تراکنش را وارد کنید"    dir="rtl"
    />
    <button
      onClick={async () => {
        try {
