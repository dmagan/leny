...
`https://p30s.com/wp-json/wp/v2/posts?_embed&order=desc&orderby=date&per_page=10&page=${pageNumber}&categories=112`,
      {
        headers: {
    ...
    if (pageNumber === 1) {
      ...
      if (data.length > 0) {
        ...
        postsNotificationService.markAllAsRead();
      }
    } else {
...
useEffect(() => {
  ...
  return () => {
    ...
    if (posts.length > 0) {
      ...
      postsNotificationService.markAllAsRead(latestPostId);
    } else {
      postsNotificationService.markAllAsRead();
    }
  };
